...
interface SuggestionImageCardProps {
  product: ProductInterface
} 

const SuggestionImageCard = memo(({product} : SuggestionImageCardProps) => {
        ...
        <Grid item xs={9} position={'relative'} minHeight={'100%'} minWidth={'100%'}>
          <Link to={`/shop/view-all/${product.id}`} aria-label={`link-to-${product.title}`}>
            <Box sx={{ width: '100%' }}>
              <CardMedia 
                component="img"
                sx={{ width: '100%', minHeight: '40vh', maxHeight: '40vh',objectFit: 'contain' }}
                image={product.image}
                alt={product.title}
...
export default SuggestionImageCard